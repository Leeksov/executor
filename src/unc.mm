...
static NSString *unc_path(lua_State *L, const char *rel) {
    NSString *p = [NSString stringWithUTF8String:(rel ? rel : "")];
    NSString *full = [[unc_workspace() stringByAppendingPathComponent:p] stringByStandardizingPath];
    NSString *wsResolved = [unc_workspace() stringByStandardizingPath];
    if (![full hasPrefix:wsResolved])
        luaL_error(L, "path escapes workspace");
    return full;
}

static int unc_readfile(lua_State *L) {
    const char *rel = luaL_checkstring(L, 1);
    NSData *d = [NSData dataWithContentsOfFile:unc_path(L, rel)];
...
static int unc_writefile(lua_State *L) {
    ...
    [[NSData dataWithBytes:data length:n] writeToFile:unc_path(L, rel) atomically:YES];
    return 0;
}
...
static int unc_isfile(lua_State *L) {
    BOOL dir = NO;
    BOOL e = [[NSFileManager defaultManager] fileExistsAtPath:unc_path(L, luaL_checkstring(L, 1)) isDirectory:&dir];
...
static int unc_makefolder(lua_State *L) {
    [[NSFileManager defaultManager] createDirectoryAtPath:unc_path(L, luaL_checkstring(L, 1))
                              withIntermediateDirectories:YES attributes:nil error:nil];
    return 0;
}

static int unc_delfile(lua_State *L) {
    [[NSFileManager defaultManager] removeItemAtPath:unc_path(L, luaL_checkstring(L, 1)) error:nil];
    return 0;
}
...
static int unc_dbg_getinfo(lua_State *L) {
    ...
    if (cl->isC) {
        ...
        lua_pushinteger(L, p->linedefined);       lua_setfield(L, -2, "linedefined");
...
void register_unc(lua_State *L) {
    struct { const char *name; lua_CFunction fn; } fns[] = {
        ...
        {"getgenv", unc_getgenv}, {"getrenv", unc_getrenv}, {"getreg", unc_getreg},
