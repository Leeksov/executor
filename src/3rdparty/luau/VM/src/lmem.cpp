...
// ---- Roblox allocator redirect (set by the tweak) ----------------------------------------------
// The vendored paged allocator keys off vendored global_State page-list offsets, which do NOT match
// Roblox's global_State. When these are set, all VM allocations go through Roblox's OWN paged
// allocator (sub_43CF674 = GC objects, sub_43CF4EC = raw), so every object lands in Roblox's pages
// and is tracked by Roblox's GC. Both take (lua_State*, size, memcat) and update global->totalbytes.
GCObject* (*rbx_gco_alloc)(lua_State*, size_t, uint8_t) = nullptr;
void* (*rbx_raw_alloc)(lua_State*, size_t, uint8_t) = nullptr;

void* luaM_new_(lua_State* L, size_t nsize, uint8_t memcat)
{
    if (rbx_raw_alloc)
        return rbx_raw_alloc(L, nsize, memcat); // Roblox paged raw alloc (updates totalbytes itself)
...
GCObject* luaM_newgco_(lua_State* L, size_t nsize, uint8_t memcat)
{
    ...
    if (rbx_gco_alloc)
        return rbx_gco_alloc(L, nsize, memcat); // Roblox paged GC-object alloc (sets marked, tracked by Roblox GC)
...
{
    // When allocations are routed to Roblox's paged allocator, its objects live in Roblox's pages
    // and are reclaimed by Roblox's GC. Freeing them through the vendored page logic (wrong
    // global_State offsets) would corrupt state, so we leave reclamation to Roblox (small leak).
    if (rbx_raw_alloc)
        return;
...
void luaM_freegco_(lua_State* L, GCObject* block, size_t osize, uint8_t memcat, lua_Page* page)
{
    extern GCObject* (*rbx_gco_alloc)(lua_State*, size_t, uint8_t);
    if (rbx_gco_alloc)
        return; // Roblox GC reclaims these; see luaM_free_ note
