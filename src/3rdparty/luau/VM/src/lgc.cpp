...
// When VM allocations are routed to Roblox's paged allocator (set by the tweak), Roblox's own GC
// owns collection. The vendored collector/barriers must not touch Roblox's global_State (its GC
// list / threshold fields sit at different offsets). This gates them off.
extern GCObject* (*rbx_gco_alloc)(lua_State*, size_t, uint8_t);
#define RBX_GC_DISABLED() (rbx_gco_alloc != nullptr)
...
{
    // When VM allocations are routed to Roblox's paged allocator, Roblox's own GC owns collection.
    // Running the vendored collector here would walk Roblox's global_State with the wrong offsets.
    extern GCObject* (*rbx_gco_alloc)(lua_State*, size_t, uint8_t);
    if (rbx_gco_alloc)
        return 0;
