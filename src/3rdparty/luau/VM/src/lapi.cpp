...
lua_State* lua_newthread(lua_State* L)
{
    ...
    // Roblox's lua_Callbacks (cb) sits at a global_State offset that differs from the vendored
    // layout, so reading g->cb.userthread here would fetch garbage. When the tweak provides Roblox's
    // userthread callback directly (it allocates/inherits the 176-byte extraspace and sets
    // L1->userdata), call that instead.
    extern void (*rbx_userthread)(lua_State*, lua_State*);
    if (rbx_userthread)
        rbx_userthread(L, L1);
    else if (g->cb.userthread)
